...
public class garfieldKartScript : MonoBehaviour {
    ...
    string logString = "";
    int img = 0;
    int HEYFLOATINGPOINTHOWSITGOING = 0;
    ...
    void Start () {
        ...
        switch (spd.ToString()) {
            case "0.25": HEYFLOATINGPOINTHOWSITGOING = 15; break;
            case "0.5": HEYFLOATINGPOINTHOWSITGOING = 30; break;
            case "0.75": HEYFLOATINGPOINTHOWSITGOING = 45; break;
            case "1": HEYFLOATINGPOINTHOWSITGOING = 60; break;
            case "1.15": HEYFLOATINGPOINTHOWSITGOING = 69; break;
            case "1.25": HEYFLOATINGPOINTHOWSITGOING = 75; break;
            default:
            Debug.LogFormat("[Garfield Kart #{0}] Cannot recognize speed value {1}, report to Blan immediately.", moduleId, spd);
            break;
        }

        calc = (int) Mathf.Floor((calc * HEYFLOATINGPOINTHOWSITGOING) / 60);
	...
	void buttonPress(KMSelectable pressedButton)
    {
        ...
        if (pressedButton == Buttons[calc])
        {
            ...
            Screen.GetComponent<MeshRenderer>().material = SolveImages[img]; //keep pressing the valid button for more pictures! :D
        } else
        {
    ...
    private readonly string TwitchHelpMessage = @"!{0} select <#> [Selects the specified place]";
    #pragma warning restore 414
    IEnumerator ProcessTwitchCommand(string command)
    {
        string[] parameters = command.Split(' ');
        if (Regex.IsMatch(parameters[0], @"^\s*select\s*$", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant) || Regex.IsMatch(parameters[0], @"^\s*press\s*$", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant) || Regex.IsMatch(parameters[0], @"^\s*place\s*$", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant))
        {
